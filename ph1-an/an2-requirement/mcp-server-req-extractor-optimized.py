#!/usr/bin/env python3
"""
MCP Server: Requirements Extractor (Optimized Version)
병렬 처리와 캐싱을 통해 timeout 문제 해결
"""
...
# 캐시 디렉토리
CACHE_DIR = Path(".cache/req-extractor")
CACHE_DIR.mkdir(parents=True, exist_ok=True)


def get_cache_key(image_dir: str, pattern: str) -> str:
    """캐시 키 생성"""
    key_str = f"{image_dir}_{pattern}"
...
async def extract_text_from_images_parallel(image_dir: str, pattern: str) -> dict:
    """이미지 파일들에서 텍스트 추출 (병렬 처리)"""
    ...
    extracted_data = {
        img_file.name: text 
        for img_file, text in zip(image_files, results)
    }
...
async def analyze_all_sections_parallel(combined_text: str, sections: list[str]) -> dict:
    """모든 섹션을 병렬로 분석"""
    tasks = [
    ...
    return [
        Tool(
            ...
            description="이미지 파일에서 요구사항을 추출하여 spec-design.md 파일을 생성합니다. "
                       "병렬 처리와 캐싱을 통해 최적화되어 빠르게 동작합니다. "
                       "OpenRouter API를 통해 Claude Sonnet 4.5를 사용합니다.",
            inputSchema={
                "type": "object",
                "properties": {
                    "image_dir": {
...
async def handle_call_tool(name: str, arguments: dict | None) -> list[TextContent]:
    ...
    try:
        result_text = f"🚀 Requirements Extractor (Optimized Version)\n"
        result_text += f"{'='*60}\n\n"
        
        # 1. 캐시 확인
        cache_key = get_cache_key(image_dir, image_pattern)
        cached_data = load_from_cache(cache_key) if use_cache else None
        
        if cached_data:
            result_text += f"✅ 캐시에서 데이터 로드 (빠른 처리)\n"
            extracted_data = cached_data.get("extracted_data", {})
            combined_text = cached_data.get("combined_text", "")
        else:
            # 2. 이미지에서 텍스트 추출 (병렬 처리)
            result_text += f"🔍 이미지 파일 검색 중...\n"
            result_text += f"   경로: {image_dir}/{image_pattern}\n\n"
            
            result_text += f"⚡ 병렬 처리로 텍스트 추출 중...\n"
            extracted_data = await extract_text_from_images_parallel(image_dir, image_pattern)
            ...
            if use_cache:
                ...
                result_text += f"💾 캐시에 저장 완료\n\n"
        
        # 3. 각 섹션별로 요구사항 분석 (병렬 처리)
        sections = [
            "기본정보",
            "비기능 요구사항",
            "애플리케이션",
        ...
        result_text += f"⚡ 병렬 처리로 {len(sections)}개 섹션 분석 중...\n"
        section_data = await analyze_all_sections_parallel(combined_text, sections)
        result_text += f"✅ 모든 섹션 분석 완료\n\n"
